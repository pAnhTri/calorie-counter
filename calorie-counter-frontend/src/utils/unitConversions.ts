export const convertWeight = (weight: string, isMetric: boolean) => {
...
export const convertHeight = (height: string, isMetric: boolean) => {
  if (isMetric) {
    ...
    if (cm < 0 || isNaN(cm)) throw new Error("Invalid height input");
    ...
    return `${feet}'${inches}"`; // Return formatted height
  } else {
    // Imperial to Metric
    if (height == "") return "0";
    ...
    return cm.toFixed(2); // Return as a number rounded to 2 decimals
  }
};
